<template>
  <ZxSelect
    v-model="innerValue"
    :options="DATA_CONVERSION_TYPE_OPTIONS"
...
import { DATA_CONVERSION_TYPE_OPTIONS, DataConversionType } from '../model'

defineOptions({
  name: 'DataConversionTypeSelector'
})

interface Props {
  /** 绑定值 */
  modelValue?: DataConversionType | DataConversionType[] | null
  /** 占位符 */
  placeholder?: string
  /** 是否可清空 */
  clearable?: boolean
...
const emit = defineEmits<{
  'update:modelValue': [value: DataConversionType | DataConversionType[] | null]
  change: [value: DataConversionType | DataConversionType[] | null]
  clear: []
  blur: []
  focus: []
  'visible-change': [visible: boolean]
  'remove-tag': [value: DataConversionType]
}>()

const innerValue = computed({
  get: () => props.modelValue,
  set: (value) => emit('update:modelValue', value as DataConversionType | DataConversionType[] | null)
})
